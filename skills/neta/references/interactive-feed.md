...
```bash
# 第 1 页
pnpm start request_interactive_feed --page_index 0 > /tmp/page0.json
BIZ_TRACE_ID_0=$(cat /tmp/page0.json | jq -r '.page_data.biz_trace_id')

# 第 2 页（正确：使用第 1 页返回的）
pnpm start request_interactive_feed --page_index 1 --biz_trace_id "$BIZ_TRACE_ID_0" > /tmp/page1.json
BIZ_TRACE_ID_1=$(cat /tmp/page1.json | jq -r '.page_data.biz_trace_id')

# 第 3 页（错误：使用了第 2 页返回的，应该一直用第 1 页的）
pnpm start request_interactive_feed --page_index 2 --biz_trace_id "$BIZ_TRACE_ID_1"
```

**❌ 错误 2：完全不传 biz_trace_id**
```bash
# 每次都开启新的会话，推荐内容会不连续
pnpm start request_interactive_feed --page_index 0
pnpm start request_interactive_feed --page_index 1  # 没有 biz_trace_id
pnpm start request_interactive_feed --page_index 2  # 没有 biz_trace_id
```
...
```bash
# 缓存第一页结果
pnpm start request_interactive_feed --page_index 0 > /tmp/feed_cache.json
...
# 同时预加载下一页（后台任务）
pnpm start request_interactive_feed --page_index 1 > /tmp/page1.prefetch.json &
wait
```
...
```bash
# 开启 debug 日志查看请求参数
DEBUG=* pnpm start request_interactive_feed --page_index 0
```
...
pnpm start request_interactive_feed --page_index 1 --biz_trace_id "$BIZ_TRACE_ID"
pnpm start request_interactive_feed --page_index 2 --biz_trace_id "$BIZ_TRACE_ID"
...
pnpm start request_interactive_feed --page_index 2 --biz_trace_id "$BIZ_TRACE_ID_1"  # 错误！应该用 BIZ_TRACE_ID_0
```
